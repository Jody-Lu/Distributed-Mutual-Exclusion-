...
void Server::AcceptAndDispatch() {
  ...
  printf("Start AcceptAndDispatch()\n");
  ...
  while(1) {
    ...
    c = new Client();
	  ...
	  // Blocks here: accept() will block process here
...
void Server::SendRequestToAll() {
  ...
  MyThread::LockMutex("'SendRequestToAll()'");

    for (size_t i = 0; i < clients.size(); i++) {
      //n = send(Server::clients[i].sock, &r, sizeof(REQUEST), 0);
      //cout << n << " bytes sent." << endl;
    }

  MyThread::UnlockMutex("'SendRequestToAll()'");
}
